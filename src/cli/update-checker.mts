...
function compareStableVersions(
    v1: ReturnType<typeof parseStableVersion>,
    v2: ReturnType<typeof parseStableVersion>,
): number {
    if (!v1 || !v2) {
        return 0;
    }
...
async function fetchLatestVersion(channel: Channel): Promise<string | null> {
    try {
        const repo =
            channel === "stable" ? GITHUB_REPO_STABLE : GITHUB_REPO_NIGHTLY;
        const apiUrl =
            channel === "stable"
...
function displayUpdateNotification(
    ...
    console.log();
    console.log(
        colorText(`Update Available! (${channelName} Channel)`, "33;1"),
    ); // bold yellow
    console.log();
