...
export default defineComponent({
  ...
  methods: {
    ...
    async compileAsLibrary() {
      ...
      if (!architecture?.config?.assemblers?.some((a: any) => a.name === "CreatorCompiler")) {
        show_notification("Library compilation requires Creator assembler", "danger")
        return false
      }
...
<template>
  ...
  >
    ...
    <div v-else>
      <!-- Filename input -->
      <b-form-group label="Library filename:" label-for="library-filename" class="mb-4">
        <b-form-input
          id="library-filename"
          ...
          title="File name"
        />
        <small class="form-text text-muted">File will be saved as {{ name_binary_save || 'library' }}.yml</small>
      </b-form-group>
        ...
        <b-form-group label="Parameters:" class="mt-3">
          ...
          >
            <b-form-input
              ...
              @blur="(e: any) => updateParameterName(symbol.name, key, (e.target as HTMLInputElement).value)"
            />
            <b-form-input
        ...
        <b-form-group label="Returns:" class="mt-3">
          ...
          >
            <b-form-input
              ...
              placeholder="Name (e.g., a0)"
              style="flex: 0 0 100px"
              @blur="(e: any) => updateReturnName(symbol.name, key, (e.target as HTMLInputElement).value)"
            />
            <b-form-input
      ...
      <p class="text-muted mt-3">
        <small>
          Note: You can leave fields empty if not needed. The library will still work without help documentation.
        </small>
      </p>
    </div>
  </b-modal>
