    ...
    execution_UI_update(ret) {
      ...
      if (app._data.autoscroll === true && run_program != 1) {
        if (execution_index >= 0 && execution_index + 4 < instructions.length) {
          const id =
            '#inst_table__row_' +
            instructions[execution_index + parseInt(architecture.arch_conf[1].value) / 8].Address
          const row_pos = $(id).position()
          if (row_pos) {
  ...
  <b-container fluid>
    <b-row>
      <span class="col px-0 mr-1" v-for="(item, index) in group">
        ...
        <!-- Information popover -->
        <PopoverInfo target="info" show_instruction_help="true"></PopoverInfo>
      </span>
    </b-row>
  </b-container>
</template>
