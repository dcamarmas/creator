      ...
      $.getJSON('architecture/available_arch.json' + '?v=' + new Date().getTime(), function (cfg) {
        ...
        if (typeof Storage !== 'undefined') {
          if (localStorage.getItem('load_architectures_available') != null) {
            const aux_arch = localStorage.getItem('load_architectures_available')
    ...
    load_default_architecture(architecture_available) {
      if (typeof Storage !== 'undefined') {
        ...
        if (
          ...
          const default_architecture = localStorage.getItem('conf_default_architecture')
    ...
    load_arch_select(e) {
      ...
      for (i = 0; i < load_architectures.length; i++) {
        if (e.name == load_architectures[i].id) {
          const aux_architecture = JSON.parse(load_architectures[i].architecture)
  ...
  >
    <b-row no-gutters>
      <b-col sm="12" @click="load_arch_select(item)" class="w-100">
        <b-card-img class="rounded-0" :src="item.img" :alt="item.alt" thumbnail fluid />
      </b-col>

      <b-col sm="12" @click="load_arch_select(item)" v-if="!default_arch(item.name)">
