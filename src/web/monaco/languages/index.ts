/**
 * Assembly Language Registry for Monaco Editor
 * 
 * This module provides language definitions for various assembly languages
 * supported by the CREATOR simulator. It dynamically generates language support
 * from the loaded architecture configuration.
 */
...
export function registerAssemblyLanguages(architecture?: ArchitectureConfig) {
  ...
  try {
    ...
    if (monacoLanguages.some(lang => lang.id === languageId)) {
      registeredLanguages.add(languageId)
      console.log(`Using pre-registered custom language for: ${architectureName} (${languageId})`)
      return
    }
    ...
    monaco.languages.setMonarchTokensProvider(languageId, langDef.tokensProvider)
    ...
    console.log(`Registered Monaco language support for: ${architectureName} (${languageId})`)
  } catch (error) {
    console.error(`Failed to register Monaco language for ${architectureName}:`, error)
  }
}
