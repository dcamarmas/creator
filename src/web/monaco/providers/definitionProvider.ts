/**
 * Definition Provider for Assembly Language
 * 
 * Provides "Go to Definition" functionality for labels in assembly code.
 * When you Ctrl+Click (or Cmd+Click) on a label reference, it will jump to the label definition.
 */
 ...
 */
function isInsideComment(line: string, position: number, commentPrefix: string = '#'): boolean {
  ...
  return {
    provideReferences: (model, position, context) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const labelDefMatch = line.match(/^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:/)
        ...
        while ((match = regex.exec(line)) !== null) {
          ...
          const isDefinition = line.match(/^\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*:/) && matchStart < line.indexOf(':')
