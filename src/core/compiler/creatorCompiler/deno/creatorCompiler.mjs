 ...
 */
"use strict";
import { main_memory, status, update_binary, architecture, backup_stack_address, backup_data_address, stats, stats_value, arch, newArchitecture, ENDIANNESSARR, WORDSIZE, BYTESIZE } from "../../../core.mjs";
import { creator_ga } from "../../../utils/creator_ga.mjs";
import { set_tag_instructions, writeMultiByteValueAsWords, setAddress, setInstructions } from "../../compiler.mjs";
...
export function assembly_compiler_default(code, library) {
  ...
  const color = Color.Ansi;
  // WASM is automatically initialized in Deno version

  const arch = ArchitectureJS.from_json(JSON.stringify(architecture));
  const instructions = [];
  ...
  const library_labels = update_binary.instructions_tag
    ?.filter(x => x.globl)
    .reduce((tbl, x) => {
      tbl[x.tag] = x.addr;
      return tbl;
    ...
    for (let i = 0; i < data_mem.length; i++) {
      ...
      switch (data.data_category()) {
        case DataCategoryJS.Number:
          switch (data.type()) {
            case "float": {
              ...
              const wordSizeBytes = newArchitecture.arch_conf.WordSize /
            ...
            case "double_word": {
              ...
              const highWord = dwordValue >>
                BigInt(newArchitecture.arch_conf.WordSize);
              const lowWord = dwordValue &
                BigInt(
                  (1n <<
                    BigInt(
                      newArchitecture.arch_conf.WordSize
        ...
        case DataCategoryJS.Space: {
          ...
          let spaceType = data.data_category() === DataCategoryJS.Padding
            ? "padding"
            : "space";
          main_memory.addHint(
            addr,
  ...
  set_tag_instructions(tag_instructions); //TODO: MAKE SURE THIS WORKS
  return {
    errorcode: "",
    token: "",
    type: "",
