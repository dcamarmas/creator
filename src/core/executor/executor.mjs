...
import { bi_intToBigInt, bi_BigIntTofloat, bi_floatToBigInt } from "../utils/bigint.mjs";
    ...
    do {
                        ...
                        switch (instruction_fields[f].type) {
                            ...
                            case "offset_words":
                                ...
                                if (
                                    ...
                                    for (
                                        var sep_index = 0;
                                        sep_index < architecture.instructions[i].fields[f].startbit.length;
                                        sep_index++
                                    ) {
                                        bin =
                                            bin +
                                            instructionExec.substring(
                                                instruction_nwords * 31 - instruction_fields[f].startbit[sep_index],
                                                instruction_nwords * 32 - instruction_fields[f].stopbit[sep_index],
                                            );
                                    }
                                } else {
                                    bin = instructionExec.substring(
            ...
            for (let i = 1; i < signatureRawParts.length; i++) {
                if (signatureParts[i] == "INT-Reg" || signatureParts[i] == "Ctrl-Reg") {
                        ...
                        for (let z = architecture.components[j].elements.length - 1; z >= 0; z--) {
                            if (architecture.components[j].elements[z].name.includes(instructionExecParts[i])) {
                                ...
                                var_readings_definitions_name[signatureRawParts[i]] =
                                    "var " + signatureRawParts[i] + "_name = '" + instructionExecParts[i] + "';\n";
                    ...
                    if (signatureParts[i].includes("float")) {
                        var_readings_definitions[signatureRawParts[i]] =
                            "var " + signatureRawParts[i] + " = Number(" + instructionExecParts[i] + ");\n";
                    } else {
                        var_readings_definitions[signatureRawParts[i]] =
                            "var " + signatureRawParts[i] + " = BigInt(" + instructionExecParts[i] + ");\n";
                    }
                }
            }
        ...
        try {
            ...
            if (e instanceof SyntaxError)
                msg = "The definition of the instruction contains errors, please review it" + e.stack;
            //TODO
            else msg = e.msg;
        ...
        if (error !== 1 && execution_index < instructions.length) {
            for (let i = 0; i < instructions.length; i++) {
                ...
                if (parseInt(instructions[i].Address, 16) == pc_reg_value) {
                    ...
                    draw.success.push(execution_index);
                    break;
                } else if (i == instructions.length - 1 && status.run_program === 3) {
        ...
        if (execution_index >= instructions.length && status.run_program === 3) {
            for (let i = 0; i < instructions.length; i++) {
                draw.space.push(i);
            }
            draw.info = [];
            return packExecute(false, "The execution of the program has finished", "success", draw); //CHECK
        } else if (execution_index >= instructions.length && status.run_program != 3) {
            for (let i = 0; i < instructions.length; i++) {
                draw.space.push(i);
            }
            draw.info = [];
...
function stats_update(type) {
    ...
    for (let i = 0; i < stats.length; i++) {
        stats[i].percentage = ((stats[i].number_instructions / status.totalStats) * 100).toFixed(2);
    }
}
