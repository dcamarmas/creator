...
export var dumptextinstructions64 = [];
export var dumpdatainstructions64 = [];
export var dumplabels64           = [];
export var sectionasm64 = 0;
export var inside_label64 = 0;
export var islib64 = 0;
export var libtags64 = [];
...
if (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER) {
  ...
  Module['print'] = function (message) {
    ...
    if (islib64) {
      ...
      if (sectionasm64 === 1){
        const name_function = message.match(/^([0-9a-f]{16})\s+<(.+?)>:$/);
        libtags64.push(name_function[2]);
      }
      ...
      // console.log("En que seccion estoy: ", sectionasm64);
      var auxinsn = [];
      ...
      if (insnmatch !== null && sectionasm64 === 1) {
        const address = parseInt(insnmatch[1].trim(), 16).toString(16)//(insnmatch[1].trim().replace(/0+/,"") === "") ? "0" : insnmatch[1].trim().replace(/0+/,""); // Parte 1: dirección
        ...
        let axx = dumptextinstructions64.findIndex(sublist => sublist.includes(address));
        if (axx != -1 && sectionasm64 === 1) {
          dumptextinstructions64[axx][1] = hexInstruction;
          dumptextinstructions64[axx][2] = asmInstruction.replace(/\\t/g, ' ');
        }
        else if (sectionasm64 === 1){
          auxinsn.push(address);
          auxinsn.push(hexInstruction);
          auxinsn.push(asmInstruction.replace(/\\t/g, ' '));
          auxinsn.push(0);
          auxinsn.push("");
          dumptextinstructions64.push(auxinsn);
        }
      }
      else if (datamatch !== null && sectionasm64 === 2){
        let axx = dumpdatainstructions64.findIndex(sublist => sublist.includes(datamatch[1])); 
        if (axx !== -1 && sectionasm64 === 2) {
            dumpdatainstructions64[axx][1] = datamatch[2];
            dumpdatainstructions64[axx][2] = ""; // asmInstruction.replace(/\\t/g, ' ');
        }
        else if (sectionasm64 === 2) {
          ...
          if(auxinsn[3] === 0){
            ...
            if(/[^0-9a-fA-F]/.test(auxinsn[1])/*   auxinsn[1].includes("madd.s")*/){
              ...
              var auxda = list_data_instructions.findIndex(data => data.label === dumpdatainstructions64[dumpdatainstructions64.length -1][4]);
              var list_data_elem;
              ...
              if (typeof list_data_instructions[auxda].value === "object"){
                if (list_data_instructions[auxda].value[aux_index_value - 1].includes("-")){
                  ...
                  dumpdatainstructions64[dumpdatainstructions64.length - 1][1] = list_data_elem + dumpdatainstructions64[dumpdatainstructions64.length -1][1];
                }else { // Caso de ser el valor positivo o sin signo
                  dumpdatainstructions64[dumpdatainstructions64.length -1][1] = list_data_instructions[auxda].value[aux_index_value - 1].toString(16) + dumpdatainstructions64[dumpdatainstructions64.length -1][1];
                }
              }
            }
            else dumpdatainstructions64[dumpdatainstructions64.length -1][1] = auxinsn[1] + dumpdatainstructions64[dumpdatainstructions64.length -1][1];
            inside_label64 += 1;
          }else 
            dumpdatainstructions64.push(auxinsn);
        }
      }
      else if(labelmatch && sectionasm64 != 0){
        // console.log("Identificado:", labelmatch);
        auxinsn.push((labelmatch[1].trim().replace(/0+/,"") === "") ? "0" : labelmatch[1].trim().replace(/0+/,"")); 
        auxinsn.push("");
        auxinsn.push("");
        auxinsn.push(1);
      ...
      console.log(message);
      // console.log(dumpdatainstructions64); 
    }
...
function writeStackCookie() {
  ...
  // ever overwritten.
  SAFE_HEAP_STORE(((max) / 4) * 4, 34821223, 4);
  SAFE_HEAP_STORE((((max) + (4)) / 4) * 4, 2310721022, 4);
}
...
function checkStackCookie() {
  ...
  var cookie1 = SAFE_HEAP_LOAD(((max) / 4) * 4, 4, 1);
  var cookie2 = SAFE_HEAP_LOAD((((max) + (4)) / 4) * 4, 4, 1);
...
function findWasmBinary() {
  if (Module["locateFile"]) {
    return locateFile("objdump64.wasm");
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL("objdump64.wasm", import.meta.url).href;
}
...
/**
  ...
  switch (type) {
   ...
   case "i16":
    return SAFE_HEAP_LOAD(((ptr) / 2) * 2, 2, 0);

   case "i32":
    return SAFE_HEAP_LOAD(((ptr) / 4) * 4, 4, 0);

   case "i64":
    return HEAP64[((ptr) / 8)];

   case "float":
    return SAFE_HEAP_LOAD_D(((ptr) / 4) * 4, 4, 0);

   case "double":
    return SAFE_HEAP_LOAD_D(((ptr) / 8) * 8, 8, 0);

   case "*":
    return Number(HEAPU64[((ptr) / 8)]);
...
/**
  ...
  switch (type) {
   ...
   case "i16":
    SAFE_HEAP_STORE(((ptr) / 2) * 2, value, 2);
    break;

   case "i32":
    SAFE_HEAP_STORE(((ptr) / 4) * 4, value, 4);
    break;

   case "i64":
    HEAP64[((ptr) / 8)] = BigInt(value);
    break;

   case "float":
    SAFE_HEAP_STORE_D(((ptr) / 4) * 4, value, 4);
    break;

   case "double":
    SAFE_HEAP_STORE_D(((ptr) / 8) * 8, value, 8);
    break;

   case "*":
    HEAPU64[((ptr) / 8)] = BigInt(value);
    break;
...
var SYSCALLS = {
  ...
  writeStat(buf, stat) {
    SAFE_HEAP_STORE(((buf) / 4) * 4, stat.dev, 4);
    SAFE_HEAP_STORE((((buf) + (4)) / 4) * 4, stat.mode, 4);
    HEAPU64[(((buf) + (8)) / 8)] = BigInt(stat.nlink);
    SAFE_HEAP_STORE((((buf) + (16)) / 4) * 4, stat.uid, 4);
    SAFE_HEAP_STORE((((buf) + (20)) / 4) * 4, stat.gid, 4);
    SAFE_HEAP_STORE((((buf) + (24)) / 4) * 4, stat.rdev, 4);
    HEAP64[(((buf) + (32)) / 8)] = BigInt(stat.size);
    SAFE_HEAP_STORE((((buf) + (40)) / 4) * 4, 4096, 4);
    SAFE_HEAP_STORE((((buf) + (44)) / 4) * 4, stat.blocks, 4);
  ...
  writeStatFs(buf, stats) {
    SAFE_HEAP_STORE((((buf) + (8)) / 4) * 4, stats.bsize, 4);
    SAFE_HEAP_STORE((((buf) + (56)) / 4) * 4, stats.bsize, 4);
    SAFE_HEAP_STORE((((buf) + (16)) / 4) * 4, stats.blocks, 4);
    SAFE_HEAP_STORE((((buf) + (20)) / 4) * 4, stats.bfree, 4);
    SAFE_HEAP_STORE((((buf) + (24)) / 4) * 4, stats.bavail, 4);
    SAFE_HEAP_STORE((((buf) + (28)) / 4) * 4, stats.files, 4);
    SAFE_HEAP_STORE((((buf) + (32)) / 4) * 4, stats.ffree, 4);
    SAFE_HEAP_STORE((((buf) + (36)) / 4) * 4, stats.fsid, 4);
    SAFE_HEAP_STORE((((buf) + (64)) / 4) * 4, stats.flags, 4);
    // ST_NOSUID
    SAFE_HEAP_STORE((((buf) + (48)) / 4) * 4, stats.namelen, 4);
  },
...
var syscallGetVarargI = () => {
  ...
  var ret = SAFE_HEAP_LOAD(((+SYSCALLS.varargs) / 4) * 4, 4, 0);
  SYSCALLS.varargs += 4;
  return ret;
};
     ...
     case 5:
      {
        ...
        SAFE_HEAP_STORE((((arg) + (offset)) / 2) * 2, 2, 2);
        return 0;
      }
     ...
     case 21505:
      {
        ...
        if (stream.tty.ops.ioctl_tcgets) {
          ...
          SAFE_HEAP_STORE(((argp) / 4) * 4, termios.c_iflag || 0, 4);
          SAFE_HEAP_STORE((((argp) + (4)) / 4) * 4, termios.c_oflag || 0, 4);
          SAFE_HEAP_STORE((((argp) + (8)) / 4) * 4, termios.c_cflag || 0, 4);
          SAFE_HEAP_STORE((((argp) + (12)) / 4) * 4, termios.c_lflag || 0, 4);
          for (var i = 0; i < 32; i++) {
     ...
     case 21508:
      {
        ...
        if (stream.tty.ops.ioctl_tcsets) {
          var argp = syscallGetVarargP();
          var c_iflag = SAFE_HEAP_LOAD(((argp) / 4) * 4, 4, 0);
          var c_oflag = SAFE_HEAP_LOAD((((argp) + (4)) / 4) * 4, 4, 0);
          var c_cflag = SAFE_HEAP_LOAD((((argp) + (8)) / 4) * 4, 4, 0);
          var c_lflag = SAFE_HEAP_LOAD((((argp) + (12)) / 4) * 4, 4, 0);
          var c_cc = [];
          for (var i = 0; i < 32; i++) {
     ...
     case 21523:
      {
        ...
        if (stream.tty.ops.ioctl_tiocgwinsz) {
          ...
          var argp = syscallGetVarargP();
          SAFE_HEAP_STORE(((argp) / 2) * 2, winsize[0], 2);
          SAFE_HEAP_STORE((((argp) + (2)) / 2) * 2, winsize[1], 2);
        }
        return 0;
      }
...
function __localtime_js(time, tmPtr) {
  ...
  SAFE_HEAP_STORE(((tmPtr) / 4) * 4, date.getSeconds(), 4);
  SAFE_HEAP_STORE((((tmPtr) + (4)) / 4) * 4, date.getMinutes(), 4);
  SAFE_HEAP_STORE((((tmPtr) + (8)) / 4) * 4, date.getHours(), 4);
  SAFE_HEAP_STORE((((tmPtr) + (12)) / 4) * 4, date.getDate(), 4);
  SAFE_HEAP_STORE((((tmPtr) + (16)) / 4) * 4, date.getMonth(), 4);
  SAFE_HEAP_STORE((((tmPtr) + (20)) / 4) * 4, date.getFullYear() - 1900, 4);
  SAFE_HEAP_STORE((((tmPtr) + (24)) / 4) * 4, date.getDay(), 4);
  var yday = ydayFromDate(date) | 0;
  SAFE_HEAP_STORE((((tmPtr) + (28)) / 4) * 4, yday, 4);
  ...
  SAFE_HEAP_STORE((((tmPtr) + (32)) / 4) * 4, dst, 4);
}
...
var __tzset_js = function(timezone, daylight, std_name, dst_name) {
  ...
  SAFE_HEAP_STORE(((daylight) / 4) * 4, Number(winterOffset != summerOffset), 4);
...
function _fd_fdstat_get(fd, pbuf) {
  ...
  try {
    ...
    SAFE_HEAP_STORE((((pbuf) + (2)) / 2) * 2, flags, 2);
