 ...
 */
  ...
  var uielto_registers_edit = {
        ...
        props:      {
                      ...
                      simple2:                        { type: String, required: true }
        ...
        methods:    {
                      ...
                      edit_register_verify(evt, comp){
                        evt.preventDefault();
                        if (this._props.name.length == 0 || !this._props.name || !this._props.defValue) {
                          show_notification('Please complete all fields', 'danger') ;
                        }
                        else if(isNaN(this._props.defValue)){
                          ...
                          for(var j=0; j < architecture.components[i].elements.length; j++){
                            if(comp == architecture.components[i].elements[j].name){
                              ...
                              if(architecture.components[i].type == "control" || architecture.components[i].type == "integer"){
                                architecture.components[i].elements[j].default_value = bi_intToBigInt(this._props.defValue,10) ;
                              }
                              else{
                                if(architecture.components[i].double_precision == false){
                                  architecture.components[i].elements[j].default_value = parseFloat(this._props.defValue, 10);
                                }
                                else{
                                  ...
                                  for (var a = 0; a < architecture_hash.length; a++) {
                                    for (var b = 0; b < architecture.components[a].elements.length; b++) {
                                      ...
                                      if(architecture.components[a].elements[b].name == this._props.simple2){
                                  ...
                                  architecture.components[i].elements[j].simple_reg[1] = this._props.simple2;
                                }
                              }
        ...
        template:   '<b-modal :id ="id" ' +
                    ...
                    '      <b-form-input type="text" ' +
                    '                    v-on:input="debounce(\'defValue\', $event)" ' +
                    '                    :value="defValue" ' +
                    '                    required ' +
                    '                    placeholder="Enter default value" ' +
                    '                    :state="valid(defValue)" ' +
                    '                    size="sm" ' +
                    '                    title="Default value">' +
                    '      </b-form-input>' +
                    '    </b-form-group>' +
                    '    <b-form-group label="Properties:">' +
                    '      <b-form-checkbox-group v-model="properties" v-if="type != \'control\'">' +
