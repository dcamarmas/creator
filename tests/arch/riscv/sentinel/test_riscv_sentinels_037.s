...
.text
    ...
    main: 
            ...
            li s1, 100      # Important value that should be preserved
            li s2, 200      # Another important value
            li s3, 300      # Yet another important value
            
            # Pass parameters incorrectly - using saved registers instead of a0-a7
            li s4, 42       # WRONG: Should use a0 for first parameter
            li s5, 84       # WRONG: Should use a1 for second parameter
            
            # Call function - caller expects s0-s5 to be unchanged after call
            jal ra, bad_function
            
            # Try to use the values we set earlier - they will be corrupted!
            add t0, s1, s2  # Expected: 100 + 200 = 300, but will be wrong
    ...
    bad_function:
            ...
            li s1, 999      # WRONG: Overwrites caller's s0 (was 100)
            li s2, 888      # WRONG: Overwrites caller's s1 (was 200)
            li s3, 777      # WRONG: Overwrites caller's s2 (was 300)
            ...
            add s1, s4, s5  # WRONG: Should use a0, a1 for parameters
