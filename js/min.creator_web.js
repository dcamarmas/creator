...
function assembly_compiler(assembly){

  var ret = {update: "", status: "ok", memory:"", token: "", tokenIndex: 0, type: ""};
  ...
  first_token(assembly);

  if(get_token(assembly) == null){
    /*hide_loading();
      ...
      if(token == architecture.directives[i].name){
        switch(architecture.directives[i].action){
          case "data_segment":
            ...
            var result = data_segment_compiler(assembly);
            if(result == 0){
              change = true;
            }
            if(result == -1){
          ...
          case "code_segment":
            ...
            var result = code_segment_compiler(assembly);
            if(result == 0){
              change = true;
            }
            if(result == -1){
...
function data_segment_compiler(assembly){
        ...
        switch(architecture.directives[j].action){
          ...
          case "ascii_not_null_end":
            ...
            while(isAscii){
              ...
              /*re = new RegExp('(.)","(.)');
              if(token.search(re) != -1){
                compileError(24, token, textarea_assembly_editor.posFromIndex(tokenIndex).line);
                hide_loading();
                return -1;
              }
              ...
              console_log(re);
              if(token.search(re) == -1){
                compileError(17, "", textarea_assembly_editor.posFromIndex(tokenIndex).line);
                hide_loading();
                return -1;
              }
...
function code_segment_compiler(assembly){
  ...
  while(existsInstruction){
    ...
    for(var i = 0; i < architecture.directives.length; i++){
      if(token == architecture.directives[i].name && architecture.directives[i].action == "global_symbol"){
        next_token(assembly); // .globl *main* 
        next_token(assembly); 
        token = get_token(assembly);
      }
      ...
      if(resultPseudo == -3){
        for (var i = 0; i < architecture.components.length; i++){
          for (var j = 0; j < architecture.components[i].elements.length; j++){
            ...
            if(token.search(re) != -1){
              //compileError(26, (textarea_assembly_editor.posFromIndex(tokenIndex).line) + 1, textarea_assembly_editor.posFromIndex(tokenIndex).line);
  ...
  for(var i = instIndex; i < architecture.instructions.length && stopFor == false; i++){
    if(architecture.instructions[i].name != instructionParts[0]){
      ...
      if(oriInstruction.search(re) == -1){
        ...
        if(isPseudo == false){
          console_log(get_token(assembly))
        ...
        if(resultPseudo == -1){
          compileError(3, architecture.instructions[i].signatureRaw, textarea_assembly_editor.posFromIndex(tokenIndex).line);
          return -1;
        }*/
      }
              ...
              if(architecture.instructions[i].fields[a].name == signatureRawParts[j]){
                ...
                if(token.match(/^0x/)){
                  ...
                  if((numAux.toString(2)).length > fieldsLength){
                    ...
                    if(resultPseudo == -1){
                      compileError(5, token, textarea_assembly_editor.posFromIndex(tokenIndex).line);
                      return -1;
                    }

                    if(resultPseudo == -2){
                      compileError(14, "", textarea_assembly_editor.posFromIndex(tokenIndex).line);
                      return -1;
                    }
                  }*/
            ...
            for(var a = 0; a < architecture.instructions[i].fields.length; a++){
              if(architecture.instructions[i].fields[a].name == signatureRawParts[j]){
                ...
                if(token.match(/^0x/)){
                  ...
                  if(isNaN(parseInt(token)) == true && resultPseudo == -3){
                    compileError(6, token, textarea_assembly_editor.posFromIndex(tokenIndex).line);
                    return -1;
                  }
